...
pub async fn check_for_update(client: &Client, config: &Config) -> Result<CatalogResponseWithOwnedData> {

    // Await server response with hard coded timeout. Could be added to config in the future.
    let resp = client.get(format!("{}/v1/catalog", config.server_url))
    ...
    let text = resp.text().await.with_context(|| "Failed to read server response as text")?;
    ...
    let owned_update_info = CatalogResponseWithOwnedData {
        entries: update_info.0.iter().map(|entry| CatalogResponseEntryWithOwnedData {
...
pub async fn download_update(client: &Client, single_update_info: &CatalogResponseEntry<'static>, config: &Config) -> Result<PathBuf> {
    ...
    let filename = format!("update_{}_{}.bin", single_update_info.name, single_update_info.version);
    ...
    let resp = client.get(single_update_info.url)
